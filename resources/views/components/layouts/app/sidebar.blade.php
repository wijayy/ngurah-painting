    ...
    <flux:sidebar sticky stashable
        ...
        <flux:navlist>
            ...
            @if (Auth::user()->role != 'driver')
                ...
                <flux:navlist.item icon="home" :href="route('driver.index')"
                ...
                <flux:navlist.item icon="home" :href="route('komisi.index')"
                    :current="request()->routeIs('komisi.*')" wire:navigate>{{ __('Komisi') }}
                </flux:navlist.item>
                <flux:navlist.item icon="home" :href="route('pembayaran.index')"
                ...
                <flux:navlist.item icon="home" :href="route('kunjungan')"
                    :current="request()->routeIs('kunjungan')" wire:navigate>{{ __('Kunjungan (Scan)') }}
                </flux:navlist.item>
                <flux:navlist.item icon="home" :href="route('stiker.index')"
        ...
        <flux:navlist>
            ...
            <form method="POST" action="{{ route('logout') }}" class="w-full">
                @csrf
                <flux:navlist.item icon="home" type="submit">{{ __('Logout') }}</flux:navlist.item>
            </form>
            <div class="text-sm capita px-3">Masuk sebagai <span class="font-semibold capitalize">{{ Auth::user()->role }}</span> </div>
        </flux:navlist>
